...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/16 12:49:54 by asahonet          #+#    #+#             */
/*   Updated: 2022/11/26 04:18:51 by asahonet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1 || read(fd, 0, 0) < 0)
		return (0);
	buffer = readbuffer(fd, rest[fd]);
	if (!buffer)
		return (0);
	rest[fd] = read_after(buffer, gnl_strchr(buffer, '\n'));
